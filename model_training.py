# model_training.py
import torch
import torch.nn as nn

class HousePricePredictor(nn.Module):
    """
    Multimodal Architecture strictly aligned with the saved checkpoint:
    - Image Features: 2560 (Satellite + StreetView)
    - Tabular Features: 125 
    - Combined Hidden Layer: 512 + 128 = 640
    """
    def __init__(self, input_dim=None, image_dim=2560): # Forced to 2560 based on error
        super(HousePricePredictor, self).__init__()
        
        # Branch 1: Image Processing Pathway (img_net)
        # Error says it expects [512, 2560]
        self.img_net = nn.Sequential(
            nn.Linear(2560, 512),
            nn.ReLU(),
            nn.BatchNorm1d(512)
        )
        
        # Branch 2: Tabular Processing Pathway (tab_net)
        # Error says it expects [128, 125]
        self.tab_net = nn.Sequential(
            nn.Linear(125, 128),
            nn.ReLU(),
            nn.BatchNorm1d(128)
        )
        
        # Branch 3: Final Regression Head (regressor)
        # Combined size: 512 (img) + 128 (tab) = 640
        # Error says it expects [256, 640] at the start and [1, 64] at the end
        self.regressor = nn.Sequential(
            nn.Linear(640, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
        )
        
    def forward(self, x):
        # Splitting input x: First 2560 are images, next 125 are tabular
        img_features = x[:, :2560]
        tab_features = x[:, 2560:2560+125]
        
        img_out = self.img_net(img_features)
        tab_out = self.tab_net(tab_features)
        
        # Late Fusion Concatenation
        combined = torch.cat((img_out, tab_out), dim=1)
        
        return self.regressor(combined)